...
public interface KnowledgeService extends IService<Knowledge> {
    ...
    ResponseDto<Knowledge> next(Long currentId, Integer tag);

    /** 按 id 取题（已忽略的不返回），用于恢复上次浏览位置 */
    ResponseDto<Knowledge> getById(Long id);

    /** 按关键字模糊搜索题干，返回前 20 条；tag 非空时按标签过滤 */
    ResponseDto<Knowledge> search(String keyword, Integer tag);

    /** 取上一题：优先取 id 小于 currentId 的未删除记录，没有则回到最后一题；tag 非空时按标签过滤 */
    ResponseDto<Knowledge> prev(Long currentId, Integer tag);
