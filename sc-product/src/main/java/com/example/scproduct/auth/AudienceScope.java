 ...
 *   内部服务调用 / 管理员 → 不过滤
 *   顾客               → 只看上架商品（status=1）
 *   商家               → 只看自己的商品与公共商品（merchant_id IS NULL），上下架均可见
 *
...
public final class AudienceScope {
    ...
    /** 非 null 时只返回 merchant_id 等于该值或为 NULL 的记录 */
     ...
     * 顾客无管理权；管理员与内部调用不限；商家仅限自己的商品与公共商品。
     *
    ...
    public boolean canManage(Integer productMerchantId) {
        ...
        return productMerchantId == null || merchantId.equals(productMerchantId);
    }
}
